...
function heatmapChartTag(hexo, args = [], content = '') {
    ...
    const colorThemesJson = JSON.stringify(colorThemes);
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (heatmapChartDom) {
                    ...
                    function getRandomTheme() {
                        const themeKeys = Object.keys(${colorThemesJson});
                        const randomKey = themeKeys[Math.floor(Math.random() * themeKeys.length)];
                        return ${colorThemesJson}[randomKey];
                    }

                    // Use the specified color theme or default to 'random'
                    const colors = ${colorThemesJson}['${theme}'.toLowerCase()] || getRandomTheme();
