...
function calculatePostStatistics(hexo) {
    ...
    const monthlyPostsByYear = [];

    for (const year in formattedDailyPostsByYear) {
        const monthlySum = new Array(12).fill(0);
        formattedDailyPostsByYear[year].forEach(([date, value]) => {
            const month = parseInt(date.split('-')[1]) - 1;
            monthlySum[month] += value;
        });

        monthlyPostsByYear.push({
            name: year,
            value: monthlySum
        });
    }

    return { dailyPostsByYear: formattedDailyPostsByYear, monthlyPostsByYear, sortedYears, sortedTags };
}
